...
export class AuthService {
  ...
  /**
   * Cambia la contraseña de cualquier usuario (uso exclusivo de administradores).
   * No requiere la contraseña actual ni verificación por email.
   * Revoca todas las sesiones activas del usuario afectado.
   */
  async adminChangePassword(
    ...
    if (!usuario) {
      throw new BadRequestException(`Usuario con ID ${targetUserId} no encontrado`);
    }
    ...
    return {
      message: `Contraseña del usuario ${usuario.email} actualizada exitosamente`,
    };
  }
