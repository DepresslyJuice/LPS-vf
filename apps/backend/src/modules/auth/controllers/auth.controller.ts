...
import { RegisterDto, LoginDto, ChangePasswordDto, AdminChangePasswordDto } from '@/modules/auth/dto/request';
...
export class AuthController {
  ...
  async register(
    ...
    // NO se establecen cookies ni se inicia sesión automáticamente,
    // para que el administrador que crea usuarios no pierda su propia sesión.
    return {
  ...
  @ApiCookieAuth('accessToken')
  @ApiOperation({
    summary: '[Admin] Cambiar contraseña de cualquier usuario',
    description:
      'Permite a un administrador establecer una nueva contraseña para cualquier usuario sin requerir verificación por email. Revoca todas las sesiones activas del usuario afectado.',
  })
  @ApiResponse({
    status: 200,
    description: 'Contraseña actualizada exitosamente',
    schema: { example: { message: 'Contraseña del usuario user@email.com actualizada exitosamente' } },
  })
  ...
  async adminChangePassword(
    ...
    return this.authService.adminChangePassword(dto.userId, dto.newPassword);
  }
}